...
```{r setup}
devtools::load_all(".")
here::i_am("vignettes/write-lineups-vignette.Rmd")
library(collapse)
library(pipdata)
library(joyn)
library(here)
...
```{r, results='hide'}
dir <- here("refy-objects")
file_list <- fs::dir_ls(dir)

file_names <- 
  fs::path_ext_remove(file_list) |> 
  fs::path_file()

lt <- lapply(file_list, qs::qread)
names(lt) <- file_names
list2env(lt, envir = .GlobalEnv)
...
The `write_multiple_refy_dist()` also makes use of the internal `add_aux_data_attr()` function, which uses `dl_aux` to find the auxiliary data for that specific country-year which was used to create the lineup distribution. It then adds this data as attributes. These attributes can be seen below for Colombia 2018 (for more on how to load the attributes directly, see the loading lineups vignette). 
