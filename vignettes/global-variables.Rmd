...
charge of loading into memory the ad-hoc variables that are used across PIP.
...
For the sake of sake of demonstrations, let's use a temporal directory as it it
...
```{r fake-root-1, eval = FALSE}
orig_root <- Sys.getenv("PIP_ROOT_DIR")
fake_dir1 <- tempdir()
gls <- pipfun::pip_create_globals(create_dir = TRUE,
                                  root_dir = fake_dir1)
...
```{r fake-root-2, eval = FALSE}
Sys.setenv(PIP_ROOT_DIR = orig_root)

fake_dir2 <- paste0(tempdir(), "/2")
fake_dir3 <- paste0(tempdir(), "/3")

gls <- pip_create_globals(root_dir   = fake_dir2, 
                          out_dir    = fake_dir3, 
                          create_dir = TRUE,
                          vintage    = c("latest", "test"))
