...
In addition to create the global vartiables, `pip_create_globals()` creates two main folders in case they don't exist, `PIP-Data_QA` and `pip_ingestion_pipeline`. This is useful if the user does not want to work in the main PIP data folder structure.  Let's focus for now on the `pc_data` sub-directory inside `pip_ingestion_pipeline`. This folder has two other folders, `cache` and `output`. Within the latter you see a vintage folder ``r format(Sys.Date(), "%Y%m%d")`` with the form `"%Y%m%d"`.
