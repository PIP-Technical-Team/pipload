...
test_that("working as expected", {
  ...
  vd <- pipfun::pip_create_globals(vintage = vintage_lt)

  expect_equal(vd$vintage_dir, vintage_ch)

  # convert character vector
  expect_equal(pipfun::pip_create_globals(vintage = vintage_ch)$vintage_dir, vintage_ch)

  # works with data frames
  vintage_DT <- data.table::as.data.table(vintage_lt)
  vintage_DF <- as.data.frame(vintage_lt)

  expect_equal(pipfun::pip_create_globals(vintage = vintage_DT)$vintage_dir, vintage_ch)
  expect_equal(pipfun::pip_create_globals(vintage = vintage_DF)$vintage_dir, vintage_ch)


  vd <- pipfun::pip_create_globals()
...
test_that("not applicable in CI", {
  ...
  vd <- pipfun::pip_create_globals(vintage = c("new", "test"),
                     out_dir = out_dir,
                     create_dir = TRUE)
...
test_that("wrong inputs trigger error", {
  ...
  expect_error(pipfun::pip_create_globals(vintage = c("a", "b", "c")))

  expect_error(pipfun::pip_create_globals(vintage = c("latest", "b")))
  expect_error(pipfun::pip_create_globals(vintage = c("fiejf", "int")))
